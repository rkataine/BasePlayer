...
public class BBFileReader {
    ...
    public BBFileReader(String path,base.BasePlayer.BedTrack track) throws IOException {
        this(path, new SeekableFileStream(new File(path)),track);

    }

    public BBFileReader(String path, SeekableStream stream, base.BasePlayer.BedTrack track) {
        ...
        if(track.getBBheader() == null) {
        	track.setBBheader(new BBFileHeader(path, fis, fileOffset));
        }
        fileHeader = track.getBBheader();
        //fileHeader.print();
